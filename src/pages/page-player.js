...
customElements.define("page-player", class extends HTMLElement {
  ...
  addEventListeners() {
    ...
    this.progressBar.addEventListener('input', (e) => {
      if (!currentSong) return
      this.timeCurrent.innerText = formatTimestamp(e.target.value);
    })
  ...
  updatePlayerInfos() {
    if (!currentSong) return
    // infos de la chanson
    this.songTitle.innerText = currentSong.title
    this.songImage.src = currentSong.artist.image_url;
    this.songArtist.innerText = currentSong.artist.name;

    // durée de la chanson
    this.timeDuration.innerText = formatTimestamp(audioPlayer.duration);
    this.progressBar.max = audioPlayer.duration;

    this.updatePlayButton();
    this.updateCurrentTime();
    
    // console.dir(currentSong, audioPlayer);
  }

  // Mise à jour de l'affichage du temps écoulé
  updateCurrentTime() {
    if(this.updatingTime) {
      this.timeCurrent.innerText = formatTimestamp(audioPlayer.currentTime);
  ...
  updatePlayButton() {
    this.querySelector('#player-control-play span').innerText = audioPlayer.paused ? "play_arrow" : "pause";
  }
})